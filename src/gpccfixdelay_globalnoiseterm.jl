...
```
"""
function gpcc(tarray, yarray; kernel = kernel, delays = delays, iterations = iterations, seed = 1, numberofrestarts = 1, initialrandom = 5, rhomin = 0.1, rhomax = rhomax)

    # Same function as below, but easier name for user to call

    gpccfixdelay(tarray, yarray; kernel = kernel, τ = delays, iterations = iterations, seed = seed, numberofrestarts = numberofrestarts, initialrandom = initialrandom, ρmin = rhomin, ρmax = rhomax)


end


function gpccfixdelay(tarray, yarray; kernel = kernel, τ = τ, iterations = iterations, seed = 1, numberofrestarts = 1, initialrandom = 5, ρmin = 0.1, ρmax = 20.0)
    ...
    function unpack(param)
        ...
        @assert(length(param) == L + 2)

        local α = makeα.(param[1:L])

        local ρ = makeρ(param[L+1])

        local σ² = makeσ²(param[L+2])
    ...
    function getsolution(i)
        ...
        local randomsolutions = [[invmakepositive.(sampleα()); invtransformbetween(initialρvalues[i], ρmin, ρmax); invmakepositive(sampleσ²())] for _ in 1:initialrandom]
    ...
    Σpostb = (Σb\I + Q'*((σ²*I + K)\Q)) \ I

    μpostb = Σpostb * ((Q' / (σ²*I + K))*Y + Σb\μb)

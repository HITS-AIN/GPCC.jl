...
```
"""
function gpcc(tarray, yarray, stdarray; minimumSqLen = 0.1, numberofrestarts = 1, iterations = 100, seed = 1)
    ...
    #---------------------------------------------------------------------
        ...
        local delays = [0.0; exp.(param[mark + 1:mark + L-1])]
    ...
    #---------------------------------------------------------------------
    # Negative objective wrapped in function that catches posdef exception
    #---------------------------------------------------------------------
    ...
    getsolution(x) = optimize(safeobj, x, NelderMead(), Optim.Options(show_trace = true, iterations = iterations, show_every = 100, g_tol=1e-6), autodiff=:forward)

    samplescale() = rand(rg)*(5-minimumScale) + minimumScale

    sampleℓ²()    = rand(rg)*(10-minimumSqLen) + minimumSqLen

    initialrandomsolutions = [[log.(rand(rg, L-1)*5 .+ 0.1);
                                [log(samplescale()) for l in 1:L];
                                randn(rg, L)*10;
                                log(sampleℓ²())] for i in 1:numberofrestarts]


    results = map(getsolution, initialrandomsolutions)

    # @show [r.minimum for r in results]
